...
class BasicIDFormEmails(Document):
	...
	def validate(self):
		if self.send_email:
			self.prepare_revision_flags()
			self.send_emails_to_details()
			self.send_email = 0

	def prepare_revision_flags(self):
		"""
		Check if email already exists in BasicIDForm.
		If yes → enable revision flag in child table row.
		"""
...
# 				# Render template
# 				subject = frappe.render_template(notification.subject, {"doc": self, "row": row})
# 				message = frappe.render_template(notification.message, {"doc": self, "row": row})
