...
// 	let site_filter = page.add_field({ fieldtype: 'Link', fieldname: 'site', options: 'Site', label: 'Site', change: function () { render_board(); } });
// 	let user_filter = page.add_field({ fieldtype: 'Link', fieldname: 'assign_to', options: 'User', label: 'Assign To', change: function () { render_board(); } });
// 	let site_count_field = page.add_field({ fieldtype: 'Data', fieldname: 'site_count', label: 'Site Counts', read_only: 1 });
...
// 					app_users = r.message.users || [];
// 					draw_table(r.message.data, r.message.milestones);
// 					site_count_field.set_value(r.message.data.length);
// 				}
// 			}
// 		});
// 	}
...
// 		milestones.forEach((m, idx) => {
// 			let c = MILESTONE_COLORS[idx % MILESTONE_COLORS.length];
// 			html += `<th colspan="6" class="ms-header" style="background-color:${c.bg}; color:${c.text};">${m}</th>`;
// 		});
// 		html += '</tr><tr>';

// 		milestones.forEach(() => {
// 				['Submission Forecast Date', 'Submission Actual Date', 'Approval Forecast Date', 'Approval Actual Date', 'Remarks', 'Assign To'].forEach(col => {
// 				html += `<th style="background-color:#f0f4f8; font-size:11px; font-weight:600; color:#475569; text-align:center; text-transform:uppercase; letter-spacing:0.4px;">${col}</th>`;
// 			});
// 		});
// 		html += '</tr></thead>';
...
// 		html += '<tbody>';
// 		if (data.length === 0) {
// 			html += `<tr><td colspan="${2 + (milestones.length * 6)}" class="text-center text-muted" style="padding:20px;">No records found.</td></tr>`;
// 		}
