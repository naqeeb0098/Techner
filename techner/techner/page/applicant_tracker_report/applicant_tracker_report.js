frappe.pages['applicant-tracker-report'].on_page_load = function (wrapper) {
	...
	let f_conclusionfeedback    = page.add_field({ fieldtype: 'Data', fieldname: 'conclusionfeedback', label: 'Conclusion/Feedback', change: refresh_data });
		...
		frappe.call({
			...
			args: {
				filters: {
					...
					conclusionfeedback : f_conclusionfeedback.get_value(),
	...
	function render_table() {
		order = order.filter(f => !["job_applicant", "job_opening"].includes(f));
		let html = `
		<style>
			.atbl-wrap {
				overflow-x: auto;
		...
		<table class="atbl">
			<thead>
				<tr>
		<th class="th-id" style="min-width:50px; width:50px">Sr</th>
		...
		${order.map(f => {
			...
			if (!custom_w) {
				...
				else if (['Data','Text','Long Text','Small Text','Text Editor'].includes(m?.fieldtype)|| f === 'job_opening_title') w = '180px';
			}

			let cls = 'th-data';
			if (m?.fieldtype === 'Link')   cls = 'th-link';
			else if (m?.fieldtype === 'Select') cls = 'th-select';
			else if (['Data','Text','Long Text','Small Text','Text Editor'].includes(m?.fieldtype)|| f === 'job_opening_title') cls = 'th-text';
		...
		data.forEach((row, idx) => {
			...
			order.forEach(f => {
				...
				else if (['Data','Text','Long Text','Small Text','Text Editor'].includes(m?.fieldtype)|| f === 'job_opening_title') {
