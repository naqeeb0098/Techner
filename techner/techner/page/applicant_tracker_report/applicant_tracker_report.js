frappe.pages['applicant-tracker-report'].on_page_load = function (wrapper) {
	...
	let f_from = page.add_field({ fieldtype: 'Date', fieldname: 'from_date', label: 'From Date', change: refresh_data });
	let f_to = page.add_field({ fieldtype: 'Date', fieldname: 'to_date', label: 'To Date', change: refresh_data });
	...
	function render_table() {
		...
		if (!data.length) {
			html += `<tr><td colspan="${order.length + 1}"
